Shader "Shotake/(obsolete)PerspectiveOutlineAbs"
{
	// persp outline (abs or rel pixel)
	Properties
	{
		_Width("Outline Pixel", float) = 1
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f vert(appdata v)
			{
				...
				float2 offset = (2 * _Width * clipPosition.w) * normalize(clipNormal.xy) / _ScreenParams.xy;
